...
export const useSpeedCardsStore = create<SpeedCardsState>((set) => ({
  ...
  initGame: (words) => set((state) => {
    ...
    const activePlayerId = state.gameMode === "duel" && playerIds.length > 0 
      ? playerIds[0] 
      : myId;
  ...
  selectCard: (id) => set((state) => {
    ...
    if (state.gameMode === "duel" && state.activePlayerId !== state.myPlayerId) {
      return {};
    }
      ...
      if (isAllMatched) {
        ...
        if (playerIds.length === 2) {
          ...
          if (updatedPlayers[p1].matches > updatedPlayers[p2].matches) {
            winnerId = p1;
          } else if (updatedPlayers[p2].matches > updatedPlayers[p1].matches) {
            winnerId = p2;
          } else {
            winnerId = "draw";
          }
